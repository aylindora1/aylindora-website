...
export default function Contact() {
  ...
  return (
    <section
      ...
      style={{
        background:
          'linear-gradient(135deg, #fecaca 0%, #fde68a 33%, #a7f3d0 66%, #bae6fd 100%)',
      }}
    >
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16 md:py-24">
        ...
        <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-black text-ink mb-3">
          {t.headline1}
          <span className="text-violet-500">{t.headline2}</span>
        </h2>
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-start w-full max-w-sm mx-auto md:max-w-3xl md:mx-0">
          <form
            action={`mailto:${cv.contactEmail}`}
            method="POST"
            encType="text/plain"
            className="min-w-0 space-y-4 bg-white border-thick border-ink rounded-3xl p-4 sm:p-6 shadow-pop-sm sm:shadow-pop"
          >
            <div>
              <label
                className="block text-xs font-black uppercase tracking-wider text-ink mb-1.5"
                htmlFor="name"
              >
                {t.formName}
              </label>
              ...
              <input
                ...
                required
                className="w-full min-w-0 px-4 py-2.5 rounded-xl border-thick border-ink bg-cream text-sm text-ink placeholder:text-ink/30 font-medium focus:outline-none focus:bg-amber-100 transition"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                required
                className="w-full min-w-0 px-4 py-2.5 rounded-xl border-thick border-ink bg-cream text-sm text-ink placeholder:text-ink/30 font-medium focus:outline-none focus:bg-amber-100 transition resize-none"
              />
            </div>
            ...
            <div className="bg-white border-thick border-ink rounded-3xl p-4 sm:p-5 shadow-pop-sm sm:shadow-pop space-y-4">
              <div>
                ...
                <div className="flex items-center min-w-0">
                  <span className="text-ink font-bold text-sm break-all min-w-0">
                    {cv.contactEmail}
                  </span>
                  ...
                  return (
                    <a
                      ...
                      className={`flex items-center justify-between gap-3 ${
                        colors[i % 3]
                      } border-thick border-ink rounded-xl px-3 py-2 text-ink text-sm font-bold min-w-0`}
                    >
                      <span className="shrink-0">{s.label}</span>

                      <span className="text-ink/40 text-xs truncate min-w-0">
